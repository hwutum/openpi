...
def make_piper_example() -> dict:
    ...
    return {
        "state": np.ones((14,)),
        "images": {
            "cam_top": np.random.randint(256, size=(3, 480, 640), dtype=np.uint8),
            "cam_side": np.random.randint(256, size=(3, 480, 640), dtype=np.uint8),
...
class PiperInputs(transforms.DataTransformFn):
    ...
    EXPECTED_CAMERAS: ClassVar[tuple[str, ...]] = ("cam_top", "cam_side", "cam_wrist")
